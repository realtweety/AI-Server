...
# ---------------------------------------------------------------------------
def maybe_prune(messages, context_limit):
...
def handle_start_stream(data):
    ...
    def do_stream(attempt=0):
        ...
        if use_web_tools:
            try:
                ...
                logger.info(f'Web tools enabled: {len(TOOL_DEFINITIONS)} tools injected')
            except ImportError:
                logger.warning('Web tools enabled but app/tools/registry.py not found')
        ...
        try:
            ...
            if attempt == 0:
                ...
                time.sleep(1.5)
                return do_stream(attempt=1)
            emit('stream_error', {'error': friendly_error(exc=e)})
            return False

    # -----------------------------------------------------------------------
    # Fix #1: Tool execution loop — actually run tools and feed results back
    # -----------------------------------------------------------------------
    MAX_TOOL_ROUNDS = 5
    ...
    tool_round = 0
    while True:
        current_tool_calls = [tool_calls_map[k] for k in sorted(tool_calls_map)]

        # No tool calls, or web tools not enabled, or max rounds reached — we're done
        if not current_tool_calls or not use_web_tools or tool_round >= MAX_TOOL_ROUNDS:
            break

        tool_round += 1
        logger.info(f'Tool round {tool_round}: executing {[tc["function"]["name"] for tc in current_tool_calls]}')

        # Tell the frontend tools are running so it can show a status indicator
        emit('stream_tool_activity', {
            'phase': 'start',
            'names': [tc['function']['name'] for tc in current_tool_calls],
            'round': tool_round,
        })
        ...
        # Execute every tool and append its result
        try:
            from app.tools.registry import execute_tool
        except ImportError:
            logger.error('execute_tool not importable — aborting tool loop')
            break
        ...
        for tc in current_tool_calls:
            ...
            if result.get('type') == 'image':
                # Encode as a multimodal content list so vision-capable models
                # can actually see the image.
                tool_content = json.dumps([
                    {'type': 'text',      'text': result.get('summary', 'Image fetched')},
            ...
            messages.append({
                'role':         'tool',
                'tool_call_id': tc.get('id') or f'call_{tool_round}_{name}',
                'content':      tool_content,
            })

        # Reset accumulators for the next streaming pass
        assistant_reply = ''
    ...
    final_tool_calls = [tool_calls_map[k] for k in sorted(tool_calls_map)]
    ...
    if assistant_reply.strip() or final_tool_calls:
