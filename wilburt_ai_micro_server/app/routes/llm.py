...
def handle_start_stream(data):
    ...
    use_web_tools   = bool(data.get('use_web_tools', False))
    ...
    # ---------------------------------------------------------------------------
    # Load web tools if requested
    # ---------------------------------------------------------------------------
    tool_definitions = []
    _execute_tool    = None
    if use_web_tools:
        try:
            from app.tools.registry import TOOL_DEFINITIONS, execute_tool as _execute_tool
            tool_definitions = TOOL_DEFINITIONS
        except ImportError as e:
            logger.warning(f'Web tools unavailable (missing dependencies?): {e}')

    # ---------------------------------------------------------------------------
    # Core streaming function — runs one model call and accumulates output
    # ---------------------------------------------------------------------------
    assistant_reply = ''
    tool_calls_map  = {}
    token_count     = 0
    ...
    # ---------------------------------------------------------------------------
    # Agentic tool loop — run model, execute tools, repeat until done
    # ---------------------------------------------------------------------------
    MAX_TOOL_ITERATIONS = 6
    all_tool_calls = []   # accumulate across iterations for stream_done

    for iteration in range(MAX_TOOL_ITERATIONS + 1):
        # Reset per-iteration accumulators (nonlocals picked up by do_stream)
        assistant_reply = ''
        tool_calls_map  = {}
        token_count     = 0
        word_count      = 0
        # Don't reset start_time — we want wall-clock time including tool execution

        success = do_stream()
        if not success:
            return

        iteration_tool_calls = [tool_calls_map[k] for k in sorted(tool_calls_map)]
        all_tool_calls.extend(iteration_tool_calls)

        # No tool calls or tools disabled or hit iteration cap → we're done
        if not iteration_tool_calls or not _execute_tool or iteration == MAX_TOOL_ITERATIONS:
            break

        # ── Notify the client which tools are about to run ──────────────────
        emit('tool_executing', {
            'tools': [
        ...
        for tc in iteration_tool_calls:
            ...
            logger.info(f'Executing tool: {tool_name}({list(args.keys())})')
            ...
            if result.get('type') == 'image':
                # Tell the model a tool result arrived (text role)
                messages.append({
                    'role':         'tool',
                    'tool_call_id': tc['id'],
                    'content':      (
                        f"Image successfully fetched from {args.get('url', 'unknown URL')}. "
                        "It has been added to the conversation as a vision input."
                    ),
                })
                # Inject the actual image as a user-role vision message
                messages.append({
                    'role': 'user',
                    'content': [
                        {'type': 'text',
                         'text': '[System: The following image was fetched by your tool call. '
                                 'Please describe or analyse it as requested.]'},
                        {'type': 'image_url',
                         'image_url': {
                             'url': f"data:{result['mime_type']};base64,{result['data']}"
                         }},
                    ],
                })
            else:
                messages.append({
                    'role':         'tool',
                    'tool_call_id': tc['id'],
                    'content':      str(result.get('content', ''))[:10_000],
                })

    # ---------------------------------------------------------------------------
    # Finalize — emit stream_done and persist history
    # ---------------------------------------------------------------------------
    ...
    if assistant_reply.strip() or all_tool_calls:
