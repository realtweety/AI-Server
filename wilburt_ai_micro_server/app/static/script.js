...
// ─── Settings popover (replaces collapsible panel) ──────────────────────────
...
// Close settings when switching chats
const _origLoadChatB3 = loadChat;
loadChat = async function(chatId) {
    settingsPopover.classList.add('hidden');
    settingsToggleBtn.classList.remove('active');
    await _origLoadChatB3(chatId);
};
...
// ═══════════════════════════════════════════════════════════════════════════
// WEB TOOLS — toggle, socket events, and live tool-activity UI
// ═══════════════════════════════════════════════════════════════════════════

// ─── State ──────────────────────────────────────────────────────────────────
let webToolsEnabled  = false;
let toolActivityEl   = null;   // live tool-status card inside the streaming bubble
...
if (webToolsBtn) {
    webToolsBtn.addEventListener('click', () => {
        ...
        webToolsBtn.setAttribute('aria-pressed', webToolsEnabled);
        webToolsBtn.classList.toggle('web-tools-active', webToolsEnabled);
        webToolsBtn.title = webToolsEnabled
            ? 'Web access ON — click to disable'
            : 'Enable web access (search, browse, images, video)';
        showToast(webToolsEnabled ? '🌐 Web access enabled' : '🌐 Web access disabled');
    });
}

// ─── Inject use_web_tools into every stream request ─────────────────────────
const _origDoStreamRequest = doStreamRequest;
doStreamRequest = function(message, isRegenerate = false, images = []) {
    if (!socket?.connected) { showToast('WebSocket not connected.', true); return; }
    const isFirst = chatContainer.querySelectorAll('.message.user').length === 1 && !isRegenerate;
    pendingStreamMeta = { isFirst, isRegenerate, images };
    toolActivityEl = null;   // reset between requests
    setStreamingState(true);
    beginStreamUI();
    socket.emit('start_stream', {
        message,
...
// ─── Helper: human-readable tool labels ─────────────────────────────────────
function toolLabel(name) {
    return {
        web_search:           '🔍 Searching the web',
        visit_webpage:        '🌐 Reading webpage',
        fetch_image:          '🖼️ Fetching image',
        get_video_transcript: '🎬 Getting video transcript',
    }[name] || `⚙️ ${name}`;
}

function toolResultLabel(name) {
    return {
        web_search:           '🔍 Search complete',
        visit_webpage:        '🌐 Page loaded',
        fetch_image:          '🖼️ Image loaded',
        get_video_transcript: '🎬 Transcript ready',
    }[name] || `✓ ${name}`;
}

// ─── Ensure tool-activity card exists inside the streaming bubble ─────────────
function ensureToolActivity() {
    ...
    toolActivityEl.className = 'tool-activity-card';
    activeStreamingBubble.appendChild(toolActivityEl);
}

// ─── Socket: tool execution starting ────────────────────────────────────────
socket.on('tool_executing', ({ tools, iteration }) => {
    ensureToolActivity();
    if (!toolActivityEl) return;

    // Add a step row for each tool call
    tools.forEach(t => {
        const row = document.createElement('div');
        row.className = 'tool-step tool-step--pending';
        row.dataset.toolKey = `${t.name}_${Date.now()}`;   // unique key
        ...
        try {
            ...
            if (first && typeof first === 'string') argSummary = `: "${first.slice(0, 60)}"`;
        } catch { /* ignore */ }

        row.innerHTML =
            `<span class="tool-step__spinner"></span>` +
            `<span class="tool-step__label">${toolLabel(t.name)}${argSummary}</span>`;

        toolActivityEl.appendChild(row);
        // Store ref on element for result pairing
        row._toolName = t.name;
    });
});

// ─── Socket: tool result arrived ────────────────────────────────────────────
socket.on('tool_result', ({ name, summary }) => {
    if (!toolActivityEl) return;

    // Find the oldest pending row for this tool name and mark it done
    const pending = toolActivityEl.querySelector(
        `.tool-step--pending[data-tool-name="${name}"]`
    );
    if (pending) {
        pending.classList.remove('tool-step--pending');
        pending.classList.add('tool-step--done');
        pending.innerHTML =
            `<span class="tool-step__check">✓</span>` +
            `<span class="tool-step__label">${toolResultLabel(name)}: <em>${summary}</em></span>`;
    }
});

// ─── When tokens arrive after tool use, slot them below the activity card ───
// Wrap the built-in stream_token handler so content goes into a separate div
const _origInitSocket2 = initSocket;
initSocket = function() {
    _origInitSocket2();

    // Patch the stream_token handler to be tool-activity-aware
    socket.off('stream_token');
    socket.on('stream_token', ({ token, word_count }) => {
        streamBuffer += token;
        if (!activeStreamingBubble) return;

        // If there is a tool activity card, render the response text below it
        if (toolActivityEl) {
            let responseDiv = activeStreamingBubble.querySelector('.tool-response-text');
            if (!responseDiv) {
                responseDiv = document.createElement('div');
                responseDiv.className = 'tool-response-text markdown';
                activeStreamingBubble.appendChild(responseDiv);
            }
            const p = separateThought(streamBuffer);
            responseDiv.innerHTML = buildBubbleHTML(p);
            attachCodeCopyButtons(responseDiv);
        } else {
            const p = separateThought(streamBuffer);
            activeStreamingBubble.innerHTML = buildBubbleHTML(p);
            attachCodeCopyButtons(activeStreamingBubble);
...
// ─── Patch finalizeStream so it handles the tool-response-text sub-div ───────
const _origFinalizeWebTools = finalizeStream;
finalizeStream = function(stats, toolCalls, pruned) {
    // If tool activity is present, the response text is in a child div —
    // move it into the bubble root so the existing finalizeStream logic works
    if (toolActivityEl && activeStreamingBubble) {
        const responseDiv = activeStreamingBubble.querySelector('.tool-response-text');
        if (responseDiv) {
            // Keep activity card, replace bubble innerHTML carefully
            const cardHTML = toolActivityEl.outerHTML;
            activeStreamingBubble.innerHTML = cardHTML;
            if (streamBuffer.trim()) {
                const p = separateThought(streamBuffer);
                const rd = document.createElement('div');
                rd.className = 'tool-response-text markdown';
                rd.innerHTML = buildBubbleHTML(p);
                activeStreamingBubble.appendChild(rd);
                attachCodeCopyButtons(rd);
            }
        }
    }
    toolActivityEl = null;
    _origFinalizeWebTools(stats, toolCalls, pruned);
};
