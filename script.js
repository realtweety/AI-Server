...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        // Dim the send button when the textarea is empty (matches Claude/ChatGPT behaviour)
        const updateSendBtn = () => {
            sendBtn.classList.toggle('disabled', messageInput.value.trim() === '');
        };
        messageInput.addEventListener('input', updateSendBtn);
        updateSendBtn(); // set initial state
    } catch (error) {
